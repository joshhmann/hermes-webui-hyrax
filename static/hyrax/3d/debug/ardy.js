...
// Capture data lives in hyrax-3d/debug/data/ and is served through the
// WebUI-origin /api/hyrax/3d/* static route, so the mirrored page under
// /static/hyrax/3d/debug/ and the API-served page share one canonical copy.
const DATA_BASE = '/api/hyrax/3d/debug/data/'
...
async function loadCapture(entry) {
  ...
  const chunks = []
  // Chunk refs may be absolute WebUI paths (e.g. /api/hyrax/3d/...), legacy
  // page-relative '../' paths, or bare files under DATA_BASE.
  for (const f of entry.chunks) {
    const url = (f.startsWith('/') || f.startsWith('../')) ? f : `${DATA_BASE}${f}`
    chunks.push(await fetchJson(url))
  }
...
async function boot() {
  ...
  const [manifest, loadedSkeleton, loadedRoleCatalog] = await Promise.all([
    ...
    fetchJson('/api/hyrax/3d/calibration-studio/contracts/soma77.skeleton.json'),
    fetchJson('/api/hyrax/3d/calibration-studio/contracts/humanoid54.authoring.json'),
  ])
  ...
  if (tpose) {
    try {
      restClip = await fetchJson(`${DATA_BASE}${tpose.chunks[0]}`)
    } catch (e) {
  ...
  try {
    ...
    for (const p of profileIndex.profiles) {
      const opt = document.createElement('option')
      opt.value = `/api/hyrax/3d/calibrate/${p.path}`
      opt.textContent = p.name
      profileSel.appendChild(opt)
    }
  } catch (e) {
    console.warn('[ardy-debug] no profile index; fallback to hardcoded', e)
    const known = ['/api/hyrax/3d/calibrate/calibration-profiles/tai-embodiment-v3.json']
  ...
  qualifiedOption.value = '/api/hyrax/3d/calibration-studio/evidence/tai.humanoid54.foot-ik.validated.json'
